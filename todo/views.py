...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request,'todo/loginuser.html',{'form':AuthenticationForm(),'error':'Неверный пароль или имя пользователя'})
        else:
            login(request,user)
            return redirect('currenttodos')
...
def createtodo(request):
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request,'todo/createtodo.html',{'form':TodoForm(),'errror':'Bad data format'})
